 ...
 */
public class EMAOvrWebInterceptor implements WebInterceptor{
    ...
    public Result intercept(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        if (!this.existsConfiguration(currentHost.getIdentifier(), request)) {
            return Result.NEXT;
        }
     ...
     */
    private Optional<String> proxyUrl(final Host currentHost, final HttpServletRequest request) {
     ...
     */
    private boolean existsConfiguration(final String hostId, final HttpServletRequest request) {
        ...
        return !APILocator.getAppsAPI().filterSitesForAppKey(EMA_APP_CONFIG_KEY,
                hosts, APILocator.systemUser()).isEmpty() || this.getProxyURL(request).isPresent();
    }

    private Optional<String> getProxyURL (final HttpServletRequest request) {
